...
# renv pacakge - create a yaml file
# See what else can be moved to helper script
...
library(gtsummary)
library(xlsx)
source("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/scripts/helper_functions.R")
library(rstatix)
library(tidytext)
library(DescTools)
library(markdown)
...
HE_scores<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/cleaned_data_v6.3.csv") %>% column_to_rownames("X") 
...
# Scaled data
HE_scaled_data<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20240510/data/clean_data/Histopathology_cleaned_filtered_scaled_data.csv") %>% column_to_rownames("X")

HE_scaled_data<-clean_column_names(HE_scaled_data,"human")


# cleaned mihc data with mean scores
mihc_scaled<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/mihc_final_with_mean_scaled_11092023.csv")

mihc_scores<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/mihc_final_with_mean_all_samples_11092023.csv")
...
# All labels 
labels<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/all_labels_cleaned") %>% mutate(sample_type = ifelse(sample_type == "SARS_CoV2_pneumonia","SCV2 Pneumonia",sample_type)) %>%
...
# Info about health condition groups for analysis - Brad
disease_list<-read_excel("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/Selected_disease_categories_for_investigation.xlsx", sheet = "approved health conditions")


# Comorbidities according to exisiting metadata
metadata_clean_health<-readxl::read_excel("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/Metadata_new_03172022.xlsx", sheet = "approved health conditions")


# All other metadata -> Age, sex, smoking etc
metadata_clean_part3<- readRDS("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/cleaned_metadata_02192024.RDS") %>%
...
CC_euc_scaled<-readRDS("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/Euclidean_scaled/CC_euc_scaled.RDS")
icl<- readRDS("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/Euclidean_scaled/ICL/icl.RDS")
#df_clus2<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/Euclidean_scaled/")
#varb_imp<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20240510/data/variable_importance/variable_importance_score.csv")

```



# Figure 1A: Comparing control to all pneumonias across features
# New type is the column name which indicates sample type (we already have sample type but we changes SARS-Cov2 to pneumonia as well in the new type and hence the new column)
...
write.csv(HE_scores_with_clust_labels_and_bac,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250224/data/HE_raw_mean_scores_with_all_labels.csv")
...
dunn_for_figure<-get_stats(t1,"new_type",type = "categorical", folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Control_vs_all_pneumonia/statistical_test_results/") %>% rstatix::add_y_position(step.increase = 0.1)


# Make plots 
#make_boxplots_dotplots(t1,varb_name = "new_type", 
#folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Control_v#s_all_pneumonia/",plot_type= "dot", width = 18,height = 16,dtype ="HE", xlab = "Sample type")
...
make_boxplots_dotplots(data_for_plot,plot_type = "bv",varb_name = "new_type", 
                       ...
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Control_vs_all_pneumonia/",
...
HE_scores_with_clust_labels_and_bac %>%
  ...
  write.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_data/mean_feature_score_grouped_by_sample_type")
```

# Features detected across samples
...
ggsave(feature_presence,filename ="/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Control_vs_all_pneumonia/feaures_present_across_pneumonic_lungs.pdf",width = 18,height =16,dpi = 300) 

ggsave(feature_presence,filename ="/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Control_vs_all_pneumonia/feaures_present_across_pneumonic_lungs.tiff",width = 18,height =16,dpi = 300) 
...
# stats
dunn_for_figure_s<-get_stats(s1,"scv2_status",folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Pneumo_vs_SARS/statistical_test_results/",type = "categorical") %>% rstatix::add_y_position(step.increase = 0.1) 
...
make_boxplots_dotplots(data_for_plot_s,plot_type = "bv",
                       ...
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Pneumo_vs_SARS/",
...
make_boxplots_dotplots(data_for_plot_ss,plot_type = "bv",
                       ...
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/Figure2A_",
...
# Correlation across all HE features only for pneumonia samples (no scv2 & control)
...
# loop over rows and columns to calculate correlation, pvalue and create a df with 0 values for insignificant correlation and actual correlation values for correlation
a = 1
for (k in 1:18) {
  b = 1
  for (j in 1:18) {
    cor_df[a,b]<-corr.test(c1[,k],c1[,j], method ="spearman", adjust = "BH")$r
    pval[a,b]<-corr.test(c1[,k],c1[,j], method ="spearman", adjust = "BH")$p.adj # make insig ones 0
    if(pval[a,b]< 0.05){
      cor_df[a,b]<- cor_df[a,b]
    }
    else if(pval[a,b]> 0.05){
      cor_df[a,b]<- 0
    }
    b = b+1
  }
...
# Plot it and store it as pdf and tiff
library(RColorBrewer)
library(corrplot)
cor_df %>%
    ggcorrplot(type = "lower", lab = TRUE,tl.cex = 25,tl.srt = 25,lab_size = 10,hc.order = TRUE) + ggstyle() +
...
fname<-paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Correlation/pneumonia_only_updated_HE_feature_correlation.tiff")
ggsave(p2,filename = fname, width = 30, height =30,units="in", dpi=300)

fname2<-paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Correlation/pneumonia_only_updated_HE_feature_correlation.pdf")
...
dunn_for_figure_b<-get_stats(b1,"Bacteria_present",
                             folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Bacterial_presence/statistical_test_results/",type = "categorical") %>% rstatix::add_y_position() %>%
  filter(feature %in% label_order) %>%
   transform(feature = factor(feature, levels = c("Abscess","Arterial\nthrombi","Bacteria","Alveolar\nHemorrhage","Edema","Necrosis","PMN","Lympho","AT2\nhyperplasia","Hyaline\nmembranes","Fibrosis")))
...
make_boxplots_dotplots(data_for_plot_b,
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Bacterial_presence/",plot_type = "bv",varb_name = "Bacteria_present", stats = dunn_for_figure_b,bx_color = c("#f03b20","#207686"), width = 16,height = 4,xlab = "Visible bacteria in the lung",dtyp = "HE")
```
# Supplemental fig 2B
...
make_boxplots_dotplots(data_for_plot_bb,stats = dunn_for_figure_bb,
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/Figure2B_",plot_type = "bv",varb_name = "Bacteria_present",bx_color = c("#f03b20","#207686"), width = 13,height = 4,xlab = "Visible bacteria in the lung",dtyp = "HE")
```
...
tiff("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Heatmaps/Human_Heatmap_HE_clusters_complexV2.tiff",width = 20,height = 18,pointsize = 20)
draw(heatmap)
dev.off()

pdf("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Heatmaps/Human_Heatmap_HE_clusters_complexV2.pdf",width = 22,height = 10,pointsize = 2)
draw(heatmap)
dev.off()
...
ggsave(cl_pl,filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Pneumo_vs_SARS/proportions_of_scv2_yes_no_across_clusters.pdf",width = 6,height = 4)
...
ggsave(bl_pl,filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Bacterial_presence/proportions_of_bacteria_yes_no_across_clusters.pdf",width = 6,height = 4)

#55505C

write.csv(fishers_result_cl,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_data/fishers_test_comparing_scv2_enrichment_across_all_clusters")
...
ggsave(x, filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Variable_importance/VI_Top10.tiff" ), width = 30, height =7, scale=1,units="in", dpi=300)
...
ggsave(pp, filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Variable_importance/VIcluster_all.tiff" ), width = 16, height =6, scale=1,units="in", dpi=300)
ggsave(pp, filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Variable_importance/VIcluster_all.pdf" ), width = 16, height =6, scale=1,units="in", dpi=300)
...
tiff("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/mclust.tiff")
plot(BIC_data)
dev.off()
...
write.csv(BIC_data,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/mclust.csv")
mclust_results<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/mclust.csv")
summary(mcresults)
```
...
ggsave(p,filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Correlation/HE_vs_mIHC_corr_test_results.tiff", width = 12, height = 20, scale=0.8,units="in", dpi=300)
ggsave(p,filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Correlation/HE_vs_mIHC_corr_test_results.pdf", width = 12, height = 20, scale=0.8,units="in", dpi=300)
...
write.csv(mihc_scores_with_clust_labels_and_bac,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/mIHC_scores_with_all_labels.csv")
...
t_test_for_figure<-get_stats(mihc_for_new_type,"new_type",
                             folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/statistical_test_results/",
...
make_boxplots_dotplots(data_for_plot_mihc_new_type,
                       ...
                       plot_type = "bvm",varb_name = "new_type",
                           folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/",
...
for (i in c("CD8","CD4","CD19","CD68","MPO")){
  ...
  mihc_for_clust %>%
    ...
    fname1<-paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/","wilcoxon_test",i,"_new_labs_continuous_human", "_boxplots_plots.tiff")
    fname2<-paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/","wilcoxon_test",i,"_new_labs_continuous_human", "_boxplots_plots.pdf")
    ...
    pl<- dd %>%
      ...
      fname1<-paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/","wilcoxon_test",i,"_logged_new_labs_continuous_human", "_boxplots_plots.tiff")
      fname2<-paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/","wilcoxon_test",i,"_logged_new_labs_continuous_human", "_boxplots_plots.pdf")
    ggsave(pl,filename = fname1, width = 8, height = 8, scale=1, unit= "in", dpi=300)
    ggsave(pl,filename = fname2, width = 8, height = 8, scale=1, unit= "in", dpi=300)
...
mfIHC_heatmap<-ComplexHeatmap::Heatmap(as.matrix(mihc_for_clust2_scaled), cell_fun = function(j, i, x, y, width, height, fill) {
        if(is.na(stat_mat2[i,j])){
         ...
         grid.text(stat_mat2[i, j], x, y, gp = gpar(fontsize = 25, col = "black"))  
        }
...
tiff("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/Human_Heatmap_mfIHC_complexV2.tiff",width = 20,height = 20,pointsize = 20)
draw(mfIHC_heatmap, merge_legend = TRUE)
dev.off()

pdf("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/mfIHC/Human_Heatmap_mfIHC_complexV2.pdf",width = 12,height =10,pointsize = 15)
draw(mfIHC_heatmap, merge_legend = TRUE)
dev.off()
...
metadata_clean_part3 %>% 
  ...
  right_join(labels %>% dplyr::select(CaseID, Primary.diagnosis, sample_type), by = c("CaseID")) %>%
  mutate(Status = ifelse(sample_type == "sample","Pneumonia",sample_type)) %>% 
  mutate(Status = ifelse(sample_type == "SARS-CoV","SARS-CoV-2",sample_type)) %>%
#  mutate(Status = ifelse(sample_type == "Control","Control",sample_type)) %>%
...
Sys.setenv(CHROMOTE_CHROME = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/scripts/Brave/brave")
gt::gtsave(test,file = file.path("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/human_demographics.pdf"))
...
mouse_HE_scaled<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/Mouse/mouse_HE_scores_cleaned__filtered_scaled.csv") %>%
  column_to_rownames("X")


# labels
mouse_labels<-read.csv("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/mouse_metadata_labels") %>%
...
#rename_clusters_mouse<-data.frame(old_labs = c(5,2,1,6,4,3),new_labs = c("A","B","C","D","E","F"))
...
mouse_cc_eu_scaled<-readRDS("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/Mouse/mouse_cc_eu_scaled.RDS")
...
mouse_models<-read_excel("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/data/Excel of mouse model corrected final 12032024.xlsx") 
...
mouse_models %>%
  ...
  mutate(Route = ifelse(Strain == "Sp19F", "intranasal",Route)) ->mouse_models
...
kableExtra::kable(mouse_models %>% select(-Pneumonia),escape = F) %>% kable_styling(full_width = F,font_size = 14)  %>%
  ...
  save_kable("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_models_table.pdf",density = 600)
...
tiff("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_Heatmap_HE_clusters_complex2.tiff",width = 20,height = 12,pointsize = 15)
ComplexHeatmap::draw(heatmap, merge_legend = TRUE)
dev.off()

pdf("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_Heatmap_HE_clusters_complex.pdf",width = 24,height = 12,pointsize = 22)
ComplexHeatmap::draw(heatmap, merge_legend = TRUE)
dev.off()
```
...
mouse_by_cluster<-mouse_labs2 %>% 
  ...
  ylab("Percentage of samples per pathogen group") +
   scale_y_continuous(labels = scales::percent) +
  labs(fill = "Pathogen")+
  theme(axis.text.x = element_text(angle = 45, hjust = 0.9, vjust = 1, size = 28,face = "bold"),strip.text = element_text(size = 16,face = "bold")) +
  theme(axis.text.y = element_text(size = 35,face = "bold"), axis.title.x.bottom = element_text(size = 35,face = "bold"), axis.title.y.left = element_text(size = 35,face = "bold"), legend.text = element_text(size =35,face = "bold"),legend.title = element_text(size =35,face = "bold"))+
  ggstyle()

ggsave(mouse_by_cluster,filename ="/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/mouse_type_by_clusters.pdf",width = 12,height =12,dpi = 300) 

ggsave(mouse_by_cluster,filename ="/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/mouse_type_by_clusters.tiff",width = 12,height =12,dpi = 300) 
```
...
mouse_labs2 %>%
  ...
  group_by(new_clust_for_fish,Pathogen) %>%
  count() %>%
  spread(new_clust_for_fish,n,fill =0) %>%
  column_to_rownames("Pathogen") %>%
  fisher_test(hybrid = TRUE,detailed = TRUE) %>%
  mutate(comparison = paste0(i,"_vs_all_other_clusters")) -> fish_mouse_path_res
  fishers_mouse_path<- rbind(fishers_mouse_path,fish_mouse_path_res)
}

write.csv(fishers_mouse_path,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_data/fishers_test_comparing_pathogen_proportions_across_clusters.csv")
...
## Alveolar hemorrhage was not averaged, but scored only by one person. 
...
write.csv(wilcox_res,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_data/mouse_saline_vs_pathogen_across_all_features_young_wilcox_test_results.csv")
...
ggplot(aes(Pathogen, score)) +
      ...
      ggstyle() -> p
  ggsave(p, filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/mouse_features_by_pathogen_group_young_boxplots.pdf", width = 24, height = 12)
```


# Comparing saline to old
...
write.csv(wilcox_res,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_data/mouse_saline_vs_pathogen_across_all_features_old_wilcox_test_results.csv")
...
ggplot(aes(Pathogen, score)) +
      ...
      geom_bracket(aes(xmin = group1,xmax =group2, label = p.adj.signif.signif,y.position = y.position),data = wilcox_res_filtered, step.group.by = "feature",step.increase = 0.01,vjust = 0.1)+
      ggstyle() -> p
  ggsave(p, filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/mouse_features_by_pathogen_group_old_boxplots.pdf", width = 20, height = 6)
```
...
pm<-mouse_HE_scaled %>%
  ...
  mutate(pathogen = trimws(pathogen,"right")) %>%
...
ggsave(pm,filename ="/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_models_HE_feature_comparison.pdf" , width = 25, height =13,units="in", dpi=300)
ggsave(pm,filename ="/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_models_HE_feature_comparison.tiff" , width = 25, height =13,units="in", dpi=300)
```
...
dunn_results<-get_stats(data_for_stats,
                         folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/",
...
mcb<-ComplexHeatmap::Heatmap(t(combined_avg2),
                                ...
                                # col =  colorRamp2( c(0, 0.25,0.5, 1), c("blue", "#e0d1c5","#e0c78d", "red")),
                               col =  colorRamp2( c(0,0.25, 0.5,0.75, 1), c("#F9EDEC","#F0D2D0","#E2A5A1", "#9f3172","#71134a")),
                                 column_title ="Samples", row_title = "Histopathology Features", 
                                # column_split = 5, 
...
pdf("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_Human_average_clustering.pdf",width = 15,height = 10,pointsize = 20)
ComplexHeatmap::draw(mcb, merge_legend = TRUE)
dev.off()


tiff("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_Human_average_clustering.tiff",width = 15,height = 10,pointsize = 20)
ComplexHeatmap::draw(mcb, merge_legend = TRUE)
dev.off()
...
for (i in 3:10){
  ...
  #  ggsave(p,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/cluster_meterics","_",i,".pdf"))
#plot(df_clus2$k,df_clus2[[i]], ylab = name_for_plot)
}

pp<-gridExtra::grid.arrange(grobs = plots, nrow = 2)

ggsave(pp,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/cluster_metrics_all.pdf"), width = 30, height = 12)
...
write.xlsx(item_cc_final,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/item_consensus_final.xlsx")
```
...
dunn_for_figure_sm1<-get_stats(sm1,
                               folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/statistical_test_results/",
...
make_boxplots_dotplots(data_for_plot_sm1,plot_type = "bv",varb_name = "smoking_status", 
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/",
...
ggsave(cp,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/smoking_status_per_cluster.pdf"), width = 30, height = 12)
ggsave(cp,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/smoking_status_per_cluster.tiff"), width = 30, height = 12)
...
# stats
dunn_for_figure_as1<-get_stats(as1,"Alcohol_status",folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/statistical_test_results/",type = "categorical")
...
make_boxplots_dotplots(data_for_plot_as1,plot_type = "bv",varb_name = "Alcohol_status",
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/",stats = dunn_for_figure_as1,bx_color = c("#f03b20","#207686"), width = 20,height = 7,xlab = "Alcohol Status",dtype = "HE",organism = "human",nrow = 2)
...
ggsave(ap,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/alcohol_status_per_cluster.pdf"), width = 30, height = 12)
ggsave(ap,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/alcohol_status_per_cluster.tiff"), width = 30, height = 12)
...
# stats
dunn_for_figure_ag<-get_stats(ag,"Age_binarized",folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/statistical_test_results/",type = "categorical")
...
make_boxplots_dotplots(data_for_plot_ag,plot_type = "bv",varb_name = "Age_binarized",
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/",stats = dunn_for_figure_ag,bx_color = c("#f03b20","#207686"), width = 20,height = 7,xlab = "Age",dtype = "HE",organism = "human",nrow = 2)
...
ggsave(age_clust,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/age_per_cluster.pdf"), width = 30, height = 12)

ggsave(age_clust,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/age_per_cluster.tiff"), width = 30, height = 12)
...
# stats
dunn_for_figure_sx1<-get_stats(sx1,"Gender",folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/statistical_test_results/",type = "categorical")
...
make_boxplots_dotplots(data_for_plot_sx1,plot_type = "bv",varb_name = "Gender", 
                       folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/",stats = dunn_for_figure_sx1,bx_color = c("#f03b20","#207686"), width = 20,height = 7,xlab = "sex",dtype = "HE",organism = "human",nrow = 2)
...
ggsave(sx_clust,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/Sex_per_cluster.pdf"), width = 30, height = 12)
ggsave(sx_clust,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/Sex_per_cluster.tiff"), width = 30, height = 12)
...
meta_stats<-get_stats(mt,"new_labs",
                      folder_name = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/statistical_test_results/Fibrosis",
                      type = "categorical")
...
ggsave(pd1, filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Supplemental_figures/FibrosisHE.pdf", width = 25, height =15, scale=0.8 )
...
for (i in 3:9){
  name_for_plot<-names(df_clus2[i])
    ...
    ggstyle()
    plots[[name_for_plot]]<-p
    ggsave(p,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20240729/figures/Mouse/Mouse_cluster_meterics","_",i,".png"))
plot(df_clus2$k,df_clus2[[i]], ylab = name_for_plot)
}

pp<-gridExtra::grid.arrange(grobs = plots, nrow = 2)

ggsave(pp,filename = paste0("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/Mouse_cluster_metrics_all.pdf"), width = 30, height = 12)
```
#mclust
...
BIC_data<-mcresults$BIC
pdf("/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/mclust.pdf")
plot(BIC_data)
dev.off()
write.csv(BIC_data,"/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Mouse/mclust.csv")
summary(mcresults)
ggsave(p,filename = "/restricted/projectnb/montilab-p/projects/pneumonia/results/Manuscript/20250426/figures/Clustering/Mouse/cluster_metrics_mclust.png")
```
